...
| News-monitoring pipeline (6 real sources, confidence-tiered extraction/matching, review queue) | **Real, working, tested against live sources** -- 4 Boulder-specific (Boulder Reporting Lab, Daily Camera, CU Independent, Boulder Weekly) + 2 statewide supplementary. Fuzzy name matching, date-proximity gating, confidence tiers (auto-matched / suggested-pending-review / discarded), and retroactive re-matching added in the Phase 6 round after diagnosing production's real review queue -- see `docs/DATA_SOURCE_FINDINGS.md` section 4a for what that diagnosis actually found (the doc's own assumed root cause was wrong; the real one was queue noise from unfiltered general-news feeds). |
...
| Linked Justice names | **Real, working** -- a Justice's name links to their profile section on the unified "Meet the Justices" page wherever it appears (recommendation callout, attendance rows, Archive attendee lists/bylines), consistently styled. Not in the original spec -- added on request. |
| Homepage + "Meet the Justices" redesign, "CUSG Court" branding | **Real, working** -- `/` is a dedicated landing page (hero, "This Week's Pick," pull-quote, about band); the filterable docket moved to `/hearings`. Every Justice's profile now renders inline on one page instead of a click-through directory. Added in the Phase 6.2 round -- see `docs/ARCHITECTURE.md`. |
| Justice availability meter + public "fits your schedule" matching | **Real, working** -- Justices record recurring weekly free time, seen as a Justice-only red-green meter per hearing (never exposed to public users/API responses); any visitor can separately enter their own free time (browser-local, no login) and see a "Fits your schedule" badge, optionally turning it into a personalized weekly-digest subscription. Added in the Phase 6.2 round -- see `docs/ARCHITECTURE.md`. |
