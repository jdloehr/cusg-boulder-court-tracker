...
def hearings_matching_subscription(db: Session, sub: Subscription) -> list[Hearing]:
    ...
    if sub.filter_type == SubscriptionFilterType.personal_availability:
        # Phase-6.3 doc: same slot-overlap function the Justice-only
        # availability meter/heatmap use (app/availability.py), so the
        # digest and the meter can never disagree with each other.
        free_slots_by_day = load_free_slots_by_day(db, AvailabilityOwnerType.personal_subscription, sub.id)
        return [h for h in hearings if hearing_matches_slots(h.date, h.time, h.duration, free_slots_by_day)]
    return []
